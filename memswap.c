...
void memswap(register void* _a, register void* _b, register size_t _size) {
	...
	while (_size >= sizeof(size_t))		// multiple of size_t
	{
		...
		*((size_t*)_b) = temp;
		...
		_a = ((size_t*)_a) + (uint8_t)1;
		_b = ((size_t*)_b) + (uint8_t)1;
		_size -= sizeof(size_t);
	}
	...
	if (_size >= sizeof(uint64_t))
	{
		...
		*((uint64_t*)_b) = temp;
		...
		_a = ((uint64_t*)_a) + (uint8_t)1;
		_b = ((uint64_t*)_b) + (uint8_t)1;
	...
	if (_size >= sizeof(uint32_t))
	{
		...
		*((uint32_t*)_b) = temp;
		...
		_a = ((uint32_t*)_a) + (uint8_t)1;
		_b = ((uint32_t*)_b) + (uint8_t)1;
	...
	if (_size >= sizeof(uint16_t))
	{
		...
		*((uint16_t*)_b) = temp;
		...
		_a = ((uint16_t*)_a) + (uint8_t)1;
		_b = ((uint16_t*)_b) + (uint8_t)1;
	...
	while (_size >= sizeof(uint8_t))	// in case at this point _size > 1, we will use a loop to copy remaining bytes
	{
		...
		*((uint8_t*)_b) = temp;
		...
		_a = ((uint8_t*)_a) + (uint8_t)1;
		_b = ((uint8_t*)_b) + (uint8_t)1;
