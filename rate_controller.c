...
void rate_controller_init(
    ...
    biquad_filter_init_lowpass(&(fc->d_term_pid_roll_biquad_filter), d_term_pid_filter_cutoff_freq_hz, sample_rate_hz);
    biquad_filter_init_lowpass(&(fc->d_term_pid_pitch_biquad_filter), d_term_pid_filter_cutoff_freq_hz, sample_rate_hz);
    biquad_filter_init_lowpass(&(fc->d_term_pid_yaw_biquad_filter), d_term_pid_filter_cutoff_freq_hz, sample_rate_hz);
...
void rate_controller_update(
    ...
    // --- Step 5: PID control ---
    // Roll & pitch use attitude error; yaw uses rate error
    float roll_out = pid_calculate(&fc->pid_roll, target_roll_rate, sensor_roll_rate, dt);
    float pitch_out = pid_calculate(&fc->pid_pitch, target_pitch_rate, sensor_pitch_rate, dt);
    float yaw_out = pid_calculate(&fc->pid_yaw, target_yaw_rate, sensor_yaw_rate, dt);
	...
	roll_k_ff = pt1_filter_apply(&fc->ff_term_pid_roll_pt1_filter, roll_k_ff);
	pitch_k_ff = pt1_filter_apply(&fc->ff_term_pid_pitch_pt1_filter, pitch_k_ff);
	yaw_k_ff = pt1_filter_apply(&fc->ff_term_pid_yaw_pt1_filter, yaw_k_ff);

    // --- Step 6: D-term filtering ---
    float roll_d = biquad_filter_apply(&fc->d_term_pid_roll_biquad_filter, fc->pid_roll.derivative_error * fc->pid_roll_k_d);
    float pitch_d = biquad_filter_apply(&fc->d_term_pid_pitch_biquad_filter, fc->pid_pitch.derivative_error * fc->pid_pitch_k_d);
    float yaw_d = biquad_filter_apply(&fc->d_term_pid_yaw_biquad_filter, fc->pid_yaw.derivative_error * fc->pid_yaw_k_d);
