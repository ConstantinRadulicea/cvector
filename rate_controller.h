...
/* ---------------- PID OUTPUT LIMITS ------------------ */
#define CONTROLLER_PID_MAX_OUTPUT (radians(1.0f) * 500.0f)   ///< Upper limit for PID outputs (before motor mixing)
...
typedef struct rate_controller_s {
    ...
    biquad_filter_t d_term_pid_roll_biquad_filter;   ///< Biquad low-pass filter for roll D-term
    biquad_filter_t d_term_pid_pitch_biquad_filter;  ///< Biquad low-pass filter for pitch D-term
    biquad_filter_t d_term_pid_yaw_biquad_filter;    ///< Biquad low-pass filter for yaw D-term
