...
static void MagCal(float* _x, float* _y, float* _z, float _bias[3], float A_1[3][3]) {
	//h_cal = (xyz - _bias) * A^-1
...
static void RotatePoint3D(coord3D* Point, rotation3D* Rotation) {
	float a, b, c;
...
static float norm3D(float x, float y, float z) {
	return sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2));
}

// return the angles between the axis and the vector
static angles3D VectorAngles(vector3D* Vector) {
...
static float quatnorm(quaternion* q) {
	return sqrtf((q->w * q->w) + (q->x * q->x) + (q->y * q->y) + (q->z * q->z));
}

static quaternion quatconj(quaternion* q) {
    quaternion result;
    result.w =  q->w;
    result.x = -q->x;
    result.y = -q->y;
...
static quaternion quatinv(quaternion* q) {
	quaternion result;
...
static quaternion quatmultiply_scalar(quaternion *q, float scalar){
	quaternion res;
    res.w = q->w * scalar;
    res.x = q->x * scalar;
    res.y = q->y * scalar;
...
static quaternion quatmultiply(quaternion* _quat1, quaternion* _quat2) {
	quaternion res;
	float w1, x1, y1, z1;
	float w2, x2, y2, z2;
...
static quaternion quatdivide(quaternion* _quat1, quaternion* _quat2) {
	quaternion qtemp = quatinv(_quat2);
	return quatmultiply(&qtemp, _quat1);
}

static vector3D quatrotate(quaternion* q, vector3D* vect) {
...
static angles3D quat2angle(quaternion* q) {
	angles3D angles;
...
static vector3D quat2rotvec(quaternion q) {
	q = quatnormalize(&q);
...
static quaternion angle2quat(angles3D* angles) {
...
static quaternion quatadd(quaternion *L, quaternion *R){
	quaternion res;
	res.w = L->w + R->w;
	res.x = L->x + R->x;
	res.y = L->y + R->y;
...
static quaternion quatsub(quaternion *L, quaternion *R){
	quaternion res;
	res.w = L->w - R->w;
	res.x = L->x - R->x;
	res.y = L->y - R->y;
...
static quaternion quat_error(quaternion *q_current, quaternion *q_desired) {
    quaternion q_conj = quatconj(q_current);
    return quatmultiply(&q_conj, q_desired);
}
...
// ===						INTEGRALS				            ===
// ================================================================
static float trapzY(float* y, size_t dim) {
...
static float trapzXY(float* x, float* y, size_t dim) {
	float _diff, sum;
	_diff = 0.0;
	sum = 0.0;
